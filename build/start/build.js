    ...
    Logo: {
        domInit: function fn () {
        	...
        	$(window).scroll(function(){
			 	...
			 	if (scroll > 3440) {
			    	$(".Logo").css("background-color" , "rgba(0,79,219,1)");
			  	}

			  	else{
				 	$(".Logo").css("background-color" , "rgba(255,255,255,1)");  	
			  	}
			})
...
Beast.decl({
    Symbol: {
        inherits:['Grid'],
        expand: function () {
            this.append(Beast.node("logo",{__context:this}))
        }
    },
    ...
    Symbol__logo: {
        expand: function fn () {
            ...
            if (this.parentBlock().mod('FadeOut')) {
                this.mod('FadeOut', true)
            }

            if (this.parentBlock().mod('FadeIn')) {
                this.mod('FadeIn', true)
            }
        },
        ...
        domInit: function fn () {
            ...
            function check(){
                $('.symbol__logo_fadein').each(function(){

                    if($(window).scrollTop()+($(window).outerHeight()) > $(this).offset().top){
                        trig = $(window).scrollTop()+($(window).outerHeight());
                        start = $(this).offset().top;
                        end = $(this).offset().top+($(window).outerHeight()/2)+($(window).outerHeight()*.25);
                        all = end-start;
                        trigall = trig-start;
                        coef = trigall/all;
            ...
            var logotype = $('.symbol__logo_fadeout'),
                scrollContainer = $(window),
                logotypeOffset = logotype.offset().top;
            ...
            scrollContainer.scroll(function() {
                ...
                var scrollTop = $(window).scrollTop(),
                    scaleFactor = logotypeOffset + 1500,
                    scaleMax = '-0.8',
                    opacityFactor = 340,
                    scale = Math.abs(
                        Math.min((scrollTop - scaleFactor) / scaleFactor, scaleMax)
                    ),
                    opacity = Math.min(1 - scrollTop / opacityFactor);
